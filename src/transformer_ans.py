...
class Model(nn.Module):
    ...
    def forward(self, input_indices, output_indices, answer_indices):
        '''
        输入参数:
        ...
        encoder_hiddens, answer_hiddens = self.encoder(input_indices, answer_indices)
        output_indices = self.decoder(output_indices, input_indices, encoder_hiddens, answer_indices, answer_hiddens)
...
class Encoder(nn.Module):
    def __init__(self, params, vocab):
        ...
        # =========== answer encoder layer
        self.answer_embedding_encoder = nn.Embedding(2, self.params.d_model)
        self.answer_position_embedding_encoder = nn.Embedding(2, self.params.d_model)
        self.answer_encoder_layer = Answer_encoder_layer(self.params)

    def forward(self, input_indices, answer_indices):
        '''
        输入参数:
        input_indices: [batch_size, input_seq_len]
        answer_indices: [batch_size, input_seq_len]
        ...
        # =========== answer =================
        answer_self_attention_masks = self.utils.build_pad_masks(query=answer_indices, key=answer_indices)
        # answer_self_attention_masks: [batch_size, answer_seq_len, answer_seq_len]

        answer_indices = self.answer_embedding_encoder(answer_indices) * np.sqrt(self.params.d_model) + \
                        self.answer_position_embedding_encoder(answer_indices)
        # answer_indices: [batch_size, answer_seq_len, d_model]

        # only one layer for answer
        answer_indices = self.answer_encoder_layer(answer_indices,
                                      answer_self_attention_masks)
        # answer_indices: [batch_size, answer_seq_len, d_model]
...
class Decoder(nn.Module):
    ...
    def forward(self, output_indices, input_indices, encoder_hiddens, answer_indices, answer_hiddens):
        '''
        输入参数:
        output_indices: [batch_size, output_seq_len]
        input_indices: [batch_size, input_seq_len]
        ...
        # build aq masks
        aq_mutual_attention_masks = self.utils.build_pad_masks(query=output_indices, key=answer_indices)
        # aq_mutual_attention_masks: [batch_size, output_seq_len, answer_seq_len]
...
class Decoder_layer(nn.Module):
    def __init__(self, params):
        ...
        # ============ memory - question (mq) ================
        # self_attention结构
        self.mq_self_attention = Multihead_attention(self.params)

        # mutual_attention结构
        self.mq_mutual_attention = Multihead_attention(self.params)

        # FFN结构
        self.mq_feedforward_network = Feedforward_network(self.params)

        # ============ answer - question (aq) ================
        # mutual_attention结构
        self.aq_mutual_attention = Multihead_attention(self.params)

        # FFN结构
        self.aq_feedforward_network = Feedforward_network(self.params)

    def forward(self,
                output_indices, encoder_hiddens, decoder_self_attention_masks, decoder_mutual_attention_masks,
                answer_hiddens, aq_mutual_attention_masks,
                vocab):
        '''
        输入参数:
        output_indices: [batch_size, output_seq_len, d_model]
        ...
        # 经过self_attention结构
        output_indices, _, _ = self.mq_self_attention(query=output_indices,
                                                   key=output_indices,
        ...
        mq_indices, mq_attention, mq_context_vector = \
            self.mq_mutual_attention(query=output_indices,
                                  key=encoder_hiddens,
                                  value=encoder_hiddens,
                                  mask=decoder_mutual_attention_masks)
        # mq_indices: [batch_size, output_seq_len, d_model]
        # attention: [batch_size, output_seq_len, input_seq_len]

        # 经过FFN结构
        mq_indices = self.mq_feedforward_network(mq_indices)
        # mq_indices: [batch_size, output_seq_len, d_model]

        # ============ answer - question (aq) ================
        # 经过mutual_attention结构
        # q: question k & v: answer
        aq_indices, aq_attention, aq_context_vector = \
            self.aq_mutual_attention(query=output_indices,
                                  key=answer_hiddens,
                                  value=answer_hiddens,
                                  mask=aq_mutual_attention_masks)
        # aq_indices: [batch_size, output_seq_len, d_model]
        # attention: [batch_size, output_seq_len, input_seq_len]

        # 经过FFN结构
        aq_indices = self.aq_feedforward_network(aq_indices)
        # aq_indices: [batch_size, output_seq_len, d_model]

        # concat
        output_indices = mq_indices + aq_indices

        return output_indices, mq_attention, mq_context_vector

class Answer_encoder_layer(nn.Module):
    def __init__(self, params):
        super().__init__()

        self.params = params

        self.answer_self_attention_layer = Multihead_attention(self.params)

        self.answer_feedforward_network = Feedforward_network(self.params)

    def forward(self, answer_indices, answer_self_attention_masks):
        # 经过self_attention结构
        answer_indices, _, _ = self.answer_self_attention_layer(query=answer_indices,
                                                  key=answer_indices,
                                                  value=answer_indices,
                                                  mask=answer_self_attention_masks)
        # answer_indices: [batch_size, answer_seq_len, d_model]

        # 经过FFN结构
        answer_indices = self.answer_feedforward_network(answer_indices)
        # answer_indices: [batch_size, answer_seq_len, d_model]
